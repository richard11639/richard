 ...
 * - Unified price/size/amount inputs (amount = price * size)
 * - Shows recent price (last trade or simulated)
 * - Duration options: 10,20,30,40 minutes, 1 hour (60), 2 hours (120)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'place_order') {
    ...
    try {
        ...
        if ($side === 'buy') {
            ...
            $pdo->prepare("INSERT INTO transactions (user_id, currency, type, amount, meta) VALUES (?, ?, 'reserve', ?, ?)")->execute([$uid, $quote, -$reserve, json_encode(['market'=>$market['symbol'],'side'=>'buy','price'=>$price,'size'=>$size,'amount'=>$cost])]);
        } else {
...
if (empty($logged_in) && empty($uid)) {
    // For demo, auto-login or require login. We'll create a demo user if none.
    // WARNING: In production use proper auth.
    $uStmt = $pdo->query("SELECT id FROM users LIMIT 1")->fetch();
    if (!$uStmt) {
        $pw = password_hash('demo123', PASSWORD_DEFAULT);
        $pdo->prepare("INSERT INTO users (username,email,password) VALUES (?,?,?)")->execute(['demo','demo@example.com',$pw]);
    }
