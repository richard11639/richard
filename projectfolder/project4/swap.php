...
<head>
  <meta charset="UTF-8">
  <title>Crypto & Currency Swap</title>
  <style>
    ...
    select,input { width:100%; padding:10px; margin:8px 0; border-radius:6px; border:1px solid #ccc; }
    button { width:100%; padding:12px; background:#007bff; border:none; border-radius:6px;
      color:#fff; font-size:16px; cursor:pointer; }
    button:hover { background:#0056b3; }
    .result, .error, .success { padding:10px; margin:10px 0; border-radius:6px; }
    .error { background:#fdd; color:#900; font-weight:bold; }
    .success { background:#dfd; color:#060; font-weight:bold; }
    .result { background:#eef; color:#004; }
  </style>
</head>
<body>
  <div class="container">
  <a href="trading4.php">home</a>   
    <h2>Swap Crypto & Currencies</h2>
    ...
    elseif ($_POST['step']=="review") {
      ...
      if ($from == $to) {
        echo "<div class='error'>❌ You cannot swap the same currency.</div>";
        ...
        echo "<div class='result'>
          <p><b>Swap From:</b> $amount $from</p>
          <p><b>Swap To:</b> $to</p>
          <p><b>Calculated Amount:</b> ".number_format($target_amount,8)." $to</p>
          <p><b>Swap Fee (2%):</b> ".number_format($fee,8)." $to</p>
    ...
    elseif ($_POST['step']=="confirm") {
      echo "<div class='success'>✅ Swap Completed!</div>";
      echo "<p><b>{$_SESSION['amount']} {$_SESSION['from']}</b> swapped to <b>".number_format($_SESSION['final'],8)." {$_SESSION['to']}</b></p>";
      session_destroy();
      echo "<form method='POST'><input type='hidden' name='step' value='choose'><button>Swap Again</button></form>";
    }
    ?>
  </div>
</body>
