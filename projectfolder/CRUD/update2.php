...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $sql = "SELECT * FROM tbliberary WHERE liberary_id = $id";
    ...
    echo "Invalid liberary ID!";
    exit();
}

// Handle form submission
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $name = $_POST['liberary_name'];
    $price = $_POST['liberary_book'];
    $description = $_POST['book_author'];

    $stmt = $mysql->prepare("UPDATE tblproduct SET liberary_name = ?, liberary_price = ?, bookauthor = ? WHERE product_id = ?");
