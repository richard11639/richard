...
// compute final totals from DB (to avoid tampering we re-calc)
$items = [];
$ids = array_keys($cart);
$types = str_repeat('i', count($ids));
$stmt = $conn->prepare("SELECT id,name,price FROM products WHERE id IN (" . implode(',', array_fill(0, count($ids), '?')) . ")");
...
// handle proof file upload if any
$proof_filename = null;
...
$stmt = $conn->prepare("INSERT INTO orders 
  (user_id, order_type, delivery_time, address, apt, city, company, firstname, lastname, phone, payment_method, proof_filename, comments, delivery_fee, tax, subtotal, total)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
$stmt->bind_param('isssssssssssssdddd',
    $user_id,
    $order_type,
    $delivery_time_text,
    $address,
...
<div class="container checkout-panel">
  ...
  <div class="summary" style="display:flex;justify-content:space-between;margin-top:8px"><span>Total</span><span>₦<?=number_format($total,2)?></span></div>

  <h4>Order info</h4>
  <div class="small">Order type: <?=htmlspecialchars($order_type)?> • When: <?=htmlspecialchars($delivery_time_text)?></div>
  <div class="small">Payment: <?=htmlspecialchars($payment_method)?><?php if ($payment_method === 'transfer') echo " — send proof to 09021427575"; ?></div>
