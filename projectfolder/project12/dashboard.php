...
try {
    ...
    if (!$adminCheck->fetch()) {
        ...
        $pdo->prepare("INSERT INTO users (username,email,password,role) VALUES ('admin','admin@example.com',?, 'admin')")->execute([$pwd]);
    }
    $count = (int)$pdo->query("SELECT COUNT(*) FROM invest_products")->fetchColumn();
    if ($count == 0) {
        $plans = [
            [2000,700],[4000,1400],[8000,2800],[16000,5600],[32000,11200],
            [64000,22400],[128000,44800],[256000,89600],[512000,179200],[1024000,358400]
        ];
        $ins = $pdo->prepare("INSERT INTO invest_products (name, principal_amount, duration_days, daily_payout) VALUES (?, ?, ?, ?)");
        foreach ($plans as $i=>$p) {
            $ins->execute(["Machine Plan #".($i+1)." (₦".number_format($p[0],0).")",$p[0],30,$p[1]]);
        }
    }
} catch (Exception $e) {
    if (DEBUG) error_log("Seed error: ".$e->getMessage());
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_POST['action']) && $_POST['action'] === 'deposit_start') {
        require_login();
        if (!check_csrf($_POST['csrf'] ?? '')) { $_SESSION['flash']='Invalid CSRF'; header('Location:?view=deposit'); exit; }
        $amount = (float)($_POST['amount'] ?? 0);
        if ($amount < MIN_DEPOSIT_NGN) { $_SESSION['flash']="Minimum deposit is ₦".number_format(MIN_DEPOSIT_NGN,2); header('Location:?view=deposit'); exit; }
        $_SESSION['deposit_amount'] = $amount;
        header('Location:?view=deposit&step=confirm'); exit;
    }
    if (!empty($_POST['action']) && $_POST['action'] === 'deposit_confirm') {
        require_login();
        if (!check_csrf($_POST['csrf'] ?? '')) { $_SESSION['flash']='Invalid CSRF'; header('Location:?view=deposit'); exit; }
        ...
        // demo bank info - in production accept user's bank transfer proof
        $account_number = '7050672951';
        $bank_name = 'Palmpay';
        $account_name = 'Ogundele Olayinka Mary';
        try {
            $ins = $pdo->prepare("INSERT INTO deposit_requests (user_id, currency, amount, account_number, bank_name, account_name, status, created_at) VALUES (?, 'NGN', ?, ?, ?, ?, 'pending', NOW())");
            $ins->execute([$_SESSION['user_id'], $amount, $account_number, $bank_name, $account_name]);
            ...
            $userQ->execute([$_SESSION['user_id']]); $userRow = $userQ->fetch();
            $subject = "New deposit request #{$reqId}";
            $body = "<p>Deposit ID: {$reqId}<br>User: ".esc($userRow['username'])." (ID: {$_SESSION['user_id']})<br>Amount: ₦".number_format($amount,2)."</p>";
            send_mail(ADMIN_EMAIL, $subject, $body);
            $_SESSION['flash'] = "Deposit request created (#{$reqId}). Admin must approve to credit your wallet. Admin notified.";
            header('Location:?view=home'); exit;
        } catch (Exception $e) {
    ...
    if (!empty($_POST['action']) && $_POST['action'] === 'admin_approve_deposit') {
        require_admin();
        if (!check_csrf($_POST['csrf'] ?? '')) { $_SESSION['flash']='Invalid CSRF'; header('Location:?view=admin'); exit; }
        $id = (int)($_POST['deposit_id'] ?? 0);
        if (!$id) { $_SESSION['flash']='Select deposit'; header('Location:?view=admin'); exit; }
        try {
            $pdo->beginTransaction();
            ...
            ensure_wallet($pdo,$uid,'NGN');
            $w = $pdo->prepare("SELECT id FROM wallets WHERE user_id = ? AND currency = 'NGN' FOR UPDATE");
            $w->execute([$uid]); $wr = $w->fetch();
            if (!$wr) { $pdo->rollBack(); $_SESSION['flash']='Wallet missing'; header('Location:?view=admin'); exit; }
            $pdo->prepare("UPDATE wallets SET balance = balance + ?, updated_at = NOW() WHERE id = ?")->execute([$amount, $wr['id']]);
            $pdo->prepare("INSERT INTO transactions (user_id,currency,type,amount,meta) VALUES (?, 'NGN','deposit',?,?)")->execute([$uid,$amount,json_encode(['deposit_request_id'=>$id,'approved_by'=>$_SESSION['user_id']])]);
            ...
            if ($uRow) send_mail($uRow['email'], "Deposit #{$id} approved", "<p>Hello ".esc($uRow['username']).", your deposit ID {$id} for ₦".number_format($amount,2)." is approved.</p>");
            $_SESSION['flash'] = 'Deposit approved';
            header('Location:?view=admin'); exit;
        } catch (Exception $e) {
            if ($pdo->inTransaction()) $pdo->rollBack();
            $_SESSION['flash'] = 'Approve error: '.$e->getMessage();
            header('Location:?view=admin'); exit;
        }
    }
...
<body>
  ...
  <main class="container">
    ...
    <?php if($view==='admin' && $role==='admin'): ?>
      <!-- Admin dashboard -->
      <div class="grid">
        <div class="card">
          <h3>Admin Panel — Pending Deposits</h3>
          <?php
            $reqs = $pdo->query("SELECT dr.*, u.username FROM deposit_requests dr JOIN users u ON dr.user_id = u.id WHERE dr.status='pending' ORDER BY dr.created_at ASC")->fetchAll();
            if (!$reqs) echo "<div class='small'>No pending deposits</div>";
          ?>
          <form method="post">
            <input type="hidden" name="csrf" value="<?=esc($token)?>">
            ...
            <?php foreach($reqs as $r): ?>
              <div style="padding:8px;border-bottom:1px dashed #eee">
                <label><input type="radio" name="deposit_id" value="<?=intval($r['id'])?>"> #<?=intval($r['id'])?> — <?=esc($r['username'])?> — ₦<?=number_format($r['amount'],2)?> (<?=esc($r['created_at'])?>)</label>
              </div>
            <?php endforeach; ?>
    ...
    <?php else: ?>
        ...
        <div class="card">
          ...
          <?php else: ?>
            ...
            <div style="margin-top:12px" class="profile-tabs">
              ...
              <a href="?view=profile&tab=recharge">Recharge</a>
              <a href="?view=profile&tab=withdraw">Withdraw</a>
            ...
            <?php if($tab==='overview'): ?>
              ...
              <div class="menu-links">
                ...
                <a class="btn" href="?view=withdraw">Withdraw</a>
            ...
            <?php elseif($tab==='recharge'): ?>
              ...
              <?php $d = $pdo->prepare("SELECT * FROM deposit_requests WHERE user_id = ? ORDER BY created_at DESC"); $d->execute([$uid]); $ds = $d->fetchAll(); ?>
      ...
      <?php elseif ($view === 'deposit'): ?>
        <div class="grid">
          <section class="card">
            <h3>Deposit</h3>
            <?php $step = $_GET['step'] ?? 'start'; ?>
            <?php if(!$logged_in): ?><div class="small">Please login to deposit. <a href="?view=login">Login</a></div>
            <?php elseif($step === 'confirm' && !empty($_SESSION['deposit_amount'])): ?>
              ...
              <p class="small">Send to our demo account (this is a demo). Admin must approve to credit your wallet.</p>
              <form method="post"><input type="hidden" name="csrf" value="<?=esc($token)?>"><input type="hidden" name="action" value="deposit_confirm"><button class="btn">Confirm deposit request</button></form>
            <?php else: ?>
              <form method="post">
                <input type="hidden" name="csrf" value="<?=esc($token)?>">
                <input type="hidden" name="action" value="deposit_start">
                <label class="small">Amount (min ₦<?=number_format(MIN_DEPOSIT_NGN,2)?>) <input class="input" name="amount" type="number" step="0.01" required></label>
                <button class="btn">Start deposit request</button>
              </form>
            <?php endif; ?>
          </section>
          ...
          <aside class="card">
            ...
            <div class="small">Create a deposit request, transfer funds to our account (demo), and admin will approve and credit your wallet. After at least one approved deposit you may invest.</div>
          </aside>
        </div>
...
(use HTTPS, proper mailer, input validation, logging, tests, rate limiting, KYC/AML, etc).