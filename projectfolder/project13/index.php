 ...
 * (Updated: immediate deposit credit, fixed matching engine and wallet handling,
 *  removed 'Active account' stat and rely on available balance only)
 *
 * FOR LEARNING ONLY.
 */
...
function match_orders(PDO $pdo, $market_id) {
    // fee / reserve assumptions must match place_order behavior
    $maker_fee_rate = 0.001; // 0.1%
    $taker_fee_rate = 0.002; // 0.2%
    $reserve_fee_rate = 0.002; // same as max_fee_rate used when placing orders

    while (true) {
        try {
            $pdo->beginTransaction();

            // lock best buy & sell orders (single best each)
            ...
            // check that held amounts cover this trade portion
            // For buys we reserved (at order time) a "reserve" = price * size_for_order * (1 + reserve_fee_rate)
            // For the trade portion (trade_size) compute the reserve portion we should consume now:
            $reserve_for_trade = $trade_price * $trade_size * (1 + $reserve_fee_rate);
            ...
            // Buyer:
            // - reduce hold_amount by reserve_for_trade (reserve was added at order creation)
            // - credit base balance (trade_size)
            // - record transactions: trade_buy (base credited), trade_cost (quote cost), fee (quote fee)
            ...
            // record cost and fee on quote (these amounts were effectively removed from buyer when placing order via the reserve)
            ...
            // credit actual base balance to buyer (increase base wallet)
            ...
            // Seller:
            // - reduce hold_amount and deduct base sold from balance
            // - credit quote balance with proceeds minus maker_fee
            ...
            // credit quote proceeds to seller
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'register') {
        ...
        try {
            ...
            // create default NGN & USDT & BTC wallets
    ...
    // PLACE ORDER (deducting from balance immediately)
if (isset($_POST['action']) && $_POST['action'] === 'place_order') {
    ...
    if ($size <= 0) { 
        $_SESSION['flash']='Invalid size'; 
        header('Location:?view=exchange'); exit; 
    }

    $market = get_market($pdo,$market_id);
    if (!$market) { 
        $_SESSION['flash']='Market not found'; 
        header('Location:?view=exchange'); exit; 
    }
    ...
    try {
        ...
        if ($side === 'buy') {
            ...
            $wq = $pdo->prepare("SELECT id, balance FROM wallets WHERE user_id = ? AND currency = ? FOR UPDATE");
            $wq->execute([$_SESSION['user_id'],$quote]); 
            $wqr = $wq->fetch();

            if (!$wqr) {
                ensure_wallet($pdo, $_SESSION['user_id'], $quote, 1000000); // demo balance
                $wq->execute([$_SESSION['user_id'],$quote]); 
                $wqr = $wq->fetch();
            }

            if ($wqr['balance'] < $reserve) {
                $pdo->prepare("UPDATE wallets SET balance = ? WHERE id = ?")
                    ->execute([$reserve + 1000, $wqr['id']]);
                $wqr['balance'] = $reserve + 1000;
            }
            ...
            $wb = $pdo->prepare("SELECT id, balance FROM wallets WHERE user_id = ? AND currency = ? FOR UPDATE");
            $wb->execute([$_SESSION['user_id'],$base]); 
            $wbr = $wb->fetch();

            if (!$wbr) {
                ensure_wallet($pdo, $_SESSION['user_id'], $base, 1000000);
                $wb->execute([$_SESSION['user_id'],$base]); 
                $wbr = $wb->fetch();
            }
            ...
            if ($wbr['balance'] < $size) {
                $pdo->prepare("UPDATE wallets SET balance = ? WHERE id = ?")
                    ->execute([$size + 1000, $wbr['id']]);
                $wbr['balance'] = $size + 1000;
            }
        ...
        if ($pdo->inTransaction()) $pdo->rollBack();
        $_SESSION['flash']='Order error: ';
        header('Location:?view=exchange'); exit;
    }
}

    // DEPOSIT: multi-step internal flow (credit immediately when user confirms)
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'deposit_confirm') {
        ...
        // permanent account details
        $account_number = '7050672951';
        $bank_name = 'Palmpay';
        $account_name = 'Ogundele Olayinka Mary';
        try {
            // credit wallet immediately in a transaction
            $pdo->beginTransaction();
            ...
            // create deposit_request but mark approved since user confirmed (you may choose 'pending' if you still want admin review)
            $ins = $pdo->prepare("INSERT INTO deposit_requests (user_id, currency, amount, account_number, bank_name, account_name, status, created_at, approved_at) VALUES (?, ?, ?, ?, ?, ?, 'approved', NOW(), NOW())");
            $ins->execute([$uid, $currency, $amount, $account_number, $bank_name, $account_name]);
            $reqId = $pdo->lastInsertId();
            $pdo->prepare("INSERT INTO transactions (user_id, currency, type, amount, meta) VALUES (?, ?, 'deposit', ?, ?)")->execute([$uid,$currency,$amount,json_encode(['deposit_request_id'=>$reqId,'immediate'=>true])]);
            $pdo->commit();
            unset($_SESSION['deposit_amount']);
            $_SESSION['flash']="Deposit credited (#{$reqId}) — your wallet has been updated.";
            header('Location:?view=home'); exit;
        } catch (Exception $e) {
            if ($pdo->inTransaction()) $pdo->rollBack();
...
if ($logged_in) {
    ...
    foreach ($user_wallets as $w) {
        ...
        // store numeric available for logic, but format later for display
        ...
        <main>
          <div class="card">
            ...
            <div class="chart-wrap">
              ...
              <button><a href="placetrade.php">place order</a></button>
              </form>
            </div>
          </div>
      ...
      <div class="grid">
        ...
        <aside>
          <div class="card">
            ...
            <p class="small">Deposits can be credited immediately by users (demo). Admin can still approve pending requests.</p>
          </div>
        </aside>
      </div>
