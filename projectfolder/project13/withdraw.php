...
if ($step === 'details') {
    ...
    if ($balance < $amount) {
        ...
        echo "<a href='deposit.php'>Make a deposit</a>"; exit;
    }
    ...
    $fee = round($amount * 0.02, 2); // 20% fee as in your UI (you can change)
    $final = round($amount - $fee, 2);

    echo "<p class='muted'>Amount: ₦" . number_format($amount,2) . "</p>";
    echo "<p class='muted'>Fee (2.5%): ₦" . number_format($fee,2) . "</p>";
...
if ($step === 'confirm') {
    ...
    try {
        ...
        if ($wr) {
            ...
            $ins = $pdo->prepare("INSERT INTO wallets (user_id, currency, balance, hold_amount) VALUES (?, 'NGN', 0, ?)");
        ...
        echo "<p class='muted'>✅ Withdraw request submitted (ID: {$reqId}). Admin will process it shortly.</p>";
