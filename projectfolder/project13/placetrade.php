 ...
 * - Unified price/size inputs
 * - Countdown uses server unix timestamp (data-expires-ts) and displays MM:SS
 * - Price label shows quote currency (₦ or $) and size label shows base currency
 * - Duration (10/20/30) used for both buy & sell; expire stored server-side
 *
 * MODIFIED: gain set to 10% (0.10) and loss set to 15% (0.15). Removed `magnitude`
 * from market events and all usage. Settlement uses the latest trade price (or
 * order price fallback) and compares it to the order price to decide gain/loss.
 *
...
// market_events now stores only direction + start/end (no magnitude)
...
// Attempt to drop magnitude column if it exists (safe upgrade path)
try {
    $pdo->exec("ALTER TABLE market_events DROP COLUMN magnitude");
} catch (Exception $e) {
...
/* ========== Timed settlement constants & function (GAIN 10%, LOSS 15%) ========== */
$BUY_SETTLE_MINUTES = 10;
$SELL_SETTLE_MINUTES = 20;
$TIMED_PROFIT_RATE_GAIN = 0.10; // 10%
$TIMED_PROFIT_RATE_LOSS = 0.15; // 15%
$MAX_FEE_RATE = 0.002;

function get_effective_last_price(PDO $pdo, $market_id, $fallback_price) {
    // Use last trade price if available, otherwise fallback to the order price
...
/* ========== process_time_based_settlements ========== */
function process_time_based_settlements(PDO $pdo) {
    ...
    foreach ($candidates as $ord) {
        try {
            ...
            if ($side === 'buy') {
                // BUY: if last_price > order_price => GAIN, else => LOSS
                ...
                if ($release > 0) {
                    ...
                    $pdo->prepare("INSERT INTO transactions (user_id, currency, type, amount, meta) VALUES (?, ?, 'timed_settlement_release', ?, ?)")
                        ->execute([$user_id, $quote, $release, json_encode(['order_id'=>$order['id'],'remaining_size'=>$remaining_size])]);
                }
                ...
                if ($last_price > $order_price) {
                    ...
                    if ($profit > 0) {
                        ...
                        $pdo->prepare("INSERT INTO transactions (user_id, currency, type, amount, meta) VALUES (?, ?, 'timed_profit', ?, ?)")
                            ->execute([$user_id, $quote, $profit, json_encode(['order_id'=>$order['id'],'last_price'=>$last_price,'order_price'=>$order_price])]);
                    }
                } else {
                    ...
                    if ($loss > 0) {
                        ...
                        $pdo->prepare("INSERT INTO transactions (user_id, currency, type, amount, meta) VALUES (?, ?, 'timed_loss', ?, ?)")
                            ->execute([$user_id, $quote, -$deduct, json_encode(['order_id'=>$order['id'],'last_price'=>$last_price,'order_price'=>$order_price])]);
                    }
                }
                ...
                if ($releaseBase > 0) {
                    ...
                    $pdo->prepare("INSERT INTO transactions (user_id, currency, type, amount, meta) VALUES (?, ?, 'timed_settlement_release_base', ?, ?)")
                        ->execute([$user_id, $base, $releaseBase, json_encode(['order_id'=>$order['id'],'remaining_size'=>$remaining_size])]);
                }

                $proceeds = $order_price * $remaining_size;
                $wqCheck = $pdo->prepare("SELECT balance FROM wallets WHERE id = ? FOR UPDATE");
                $wqCheck->execute([$wqr['id']]); $curQuote = $wqCheck->fetch();
                ...
                if ($last_price < $order_price) {
                    ...
                    if ($profit > 0) {
                        ...
                        $pdo->prepare("INSERT INTO transactions (user_id, currency, type, amount, meta) VALUES (?, ?, 'timed_profit', ?, ?)")
                            ->execute([$user_id, $quote, $credit, json_encode(['order_id'=>$order['id'],'last_price'=>$last_price,'order_price'=>$order_price])]);
                    }
                } else {
                    ...
                    if ($credit > 0) {
                        $pdo->prepare("UPDATE wallets SET balance = balance + ? WHERE id = ?")->execute([$credit, $wqr['id']]);
                        $pdo->prepare("INSERT INTO transactions (user_id, currency, type, amount, meta) VALUES (?, ?, 'timed_proceeds', ?, ?)")
                            ->execute([$user_id, $quote, $credit, json_encode(['order_id'=>$order['id'],'order_price'=>$order_price])]);
                    }
...
/* ========== Simple matching engine (unchanged) ========== */
...
try { process_time_based_settlements($pdo); } catch (Exception $e) { /* ignore for demo */ }
    ...
    try {
        ...
        if ($side === 'buy') {
            ...
            $pdo->prepare("INSERT INTO transactions (user_id, currency, type, amount, meta) VALUES (?, ?, 'reserve', ?, ?)")
                ->execute([$uid, $quote, -$reserve, json_encode(['market'=>$market['symbol'],'side'=>'buy','price'=>$price,'size'=>$size])]);
        } else {
            ...
            $pdo->prepare("INSERT INTO transactions (user_id, currency, type, amount, meta) VALUES (?, ?, 'reserve', ?, ?)")
                ->execute([$uid, $base, -$size, json_encode(['market'=>$market['symbol'],'side'=>'sell','price'=>$price,'size'=>$size])]);
        }
        ...
        // Create market event demo (direction only — no magnitude)
        $isDown = (mt_rand(1,100) <= 60);
        if ($isDown) {
            $minutes = mt_rand(2,5);
            $direction = 'down';
        } else {
...
$openOrdersUser->execute([$uid]);
$openOrdersUser = $openOrdersUser->fetchAll();
...
// fetch orderbook snapshot for active pair (client will choose) and active events
$market_snaps = [];
$market_events_active = [];
...
<body>
      ...
      <aside class="card">
        ...
        <div id="marketList">
          <?php foreach($markets as $m): ?>
            <!-- embed base and quote so client can update labels immediately -->
            <div class="market-item" data-symbol="<?=htmlspecialchars($m['symbol'])?>" data-id="<?=intval($m['id'])?>" data-base="<?=htmlspecialchars($m['base_currency'])?>" data-quote="<?=htmlspecialchars($m['quote_currency'])?>">
          ...
          <div style="width:360px">
            <form method="post">
              ...
              <label class="small">Size
                <div style="display:flex;gap:8px">
                  <input class="input" id="sizeField" name="size" type="number" step="0.00000001" required style="flex:1">
              ...
              <div class="small" style="margin-top:8px">Price is required for both buy and sell orders. Price unit shown next to field.</div>
            </form>
        ...
        <div style="margin-top:12px">
          ...
          <table class="table">
            ...
            <?php foreach($openOrdersUser as $o):
                ...
                // server unix timestamp (seconds) for reliable countdown
  ...
  <script>
    // Client-side simulation + countdowns (MM:SS) + event-driven short moves
    const markets = <?php echo json_encode(array_column($markets,'symbol')); ?>;
    const marketMeta = {}; // map symbol -> {id, base, quote}
    document.querySelectorAll('.market-item').forEach(el=>{
      marketMeta[el.dataset.symbol] = { id: el.dataset.id, base: el.dataset.base, quote: el.dataset.quote };
    });
    ...
    const activeEvents = <?php echo json_encode($market_events_active); ?>; // events have direction only now
    ...
    function setActiveMarket(symbol, el) {
      ...
      document.getElementById('priceField').value = marketData[symbol].last;
    ...
    document.querySelectorAll('.market-item').forEach((el,idx)=>{
      ...
      el.addEventListener('click', ()=>{
        setActiveMarket(el.dataset.symbol, el);
      });
    });

    const firstEl = document.querySelector('.market-item');
    if (firstEl) {
      setActiveMarket(firstEl.dataset.symbol, firstEl);
    }
    ...
    // Update price/size labels to show currency unit
    ...
    updateCurrencyLabels(activeSymbol);
